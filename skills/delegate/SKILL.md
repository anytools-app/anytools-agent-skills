...
- **重要な対応の実装は司令塔が自分でやらない**(ユーザー方針、2026-07-20)。「小さい」「ライブ環境の検証が要る」「並行書き込み衝突」等を理由に司令塔が直接コードを書きたくなる類こそ、`codex gpt-5.6-sol / max` へ委任する(`adapters/codex.md`「重要・高リスク」)。司令塔が保持するのは調査・設計・成果物レビュー・コミットで、**コードの実装は委任**する。委任コストが割高に見えても、司令塔が最上位モデルの推論を実装に費やすより、司令塔をレビューに専念させる方が全体品質が上がる
