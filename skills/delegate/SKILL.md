...
- **重要・高リスクな対応でない**(委任インフラ・ログ健全性・リリースに関わるコード・不可逆操作・退行リスクの高い変更など)
- 広いコードリーディングを必要としない
- 指示書作成・委任・レビューのコストが実装コストを明らかに上回る

1つでも満たさない場合は、調査・詳細設計・実装・独立レビューのいずれかに委任する。挙動・仕様・生成条件に触る変更は、行数が少なくても委任+レビューの対象にする。

- **重要な対応の実装は司令塔が自分でやらない**(ユーザー方針、2026-07-20)。「小さい」「ライブ環境の検証が要る」「並行書き込み衝突」等を理由に司令塔が直接コードを書きたくなる類こそ、`codex gpt-6-astra / max` へ委任する(`adapters/codex.md`「重要・高リスク」)。司令塔が保持するのは設計・成果物レビュー・採否・コミットで、**コードの実装は委任**する。委任コストが割高に見えても、司令塔が最上位モデルの推論を実装に費やすより、司令塔をレビューに専念させる方が全体品質が上がる
- **調査も司令塔が自分でやらない**(ユーザー方針、2026-07-20)。原因調査・コードリーディング・棚卸しを司令塔が直接 grep/読解しがちだが、これも委任する。振り分けは**調査対象の場所**で決まる(Codex read-only の sandbox は `--cd`=対象リポジトリに限られるため):
  ...
  - **Web/X・最新情報** → Grok、**大規模読解・Google 検索付きドキュメント** → Antigravity
  - 司令塔は「何を調べさせ、何を返させるか」の設計と、返ってきた結果の判断・原典確認に専念する。例外は数手で終わる確認的な grep・単一ファイルの一読(委任コストが上回る場合のみ直接)
...
0. **入力種別を判定する**: ユーザーの入力が質問・調査・相談なら、成果物は回答・調査結果・評価であり、委任するとしても read-only(調査・相談)に限る。実装(ファイル変更を伴う委任・直接処理)へ進むのは、ユーザーが明示的に実装を指示した場合だけ。回答の勢いで実装に着手しない(複合指示「〜できる? できるならやって」は実装指示。迷ったら止まって確認する)
...
- **`model` は単一の enum 値**(`delegate-run --lint-log` は model を検査しないので規約で守る)。resume で上位ティアへ昇格した場合(例: terra 初回 → astra 修正)は、**採用された最終成果物を出したモデル**を `model` に記録し、昇格の経緯は `note` に書く。`gpt-5.6-terra→gpt-6-astra` のような複合値は使わない(`group_by(.model)` の集計を壊す)
