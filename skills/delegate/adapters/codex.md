...
| 重要・高リスク | 委任インフラ・ログ健全性・リリース・不可逆操作・認証/課金/セキュリティ・退行リスク大。難易度でなく**重要度**で選ぶ軸 | `-m gpt-5.6-sol -c 'model_reasoning_effort="xhigh"'` |
...
- **「重要・高リスク」ティア(sol/xhigh)は難易度でなく重要度の軸**(ユーザー方針 2026-07-20)。司令塔が「自分でやった方が早い」と感じる重要対応こそここへ委任し、司令塔はレビューに専念する(`../SKILL.md`「委任可否ゲート」)。`xhigh` は GPT-5.6 で公式 effort 表に無い値なので、`-m` 失敗時は TUI `/model` で対象モデルの現行 effort を確認し、無ければ `high`(難所ティア)へ落とす。委任ログで sol/xhigh の採用率・手戻りを評価して定着可否を判断する
