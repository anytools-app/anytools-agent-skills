...
方式は常に「ルートクラスのみハッシュ化+内部は `:global()` 子孫で原文一字一句」。バッチはクラスタ単位(半分割禁止)で、以下の2段階で進める。

### 第1段階: React専有領域(常に安全)

- 対象は Reactだけが描画する共有チューム(Header/Footer等)と専用ページ(トップ等)。バッチ選定時に fixed-pages / documents.ndjson / 全srcの**3点grep**で凍結HTML・CMS本文との共有クラスを特定する(共有クラスは第1段階では移さず、第2段階のclass-mapで扱う)
- **分割境界オーバーラップを機械検証してから移す**: 移動ルールと残置ルール(同ファイル残り+共有レガシー)の間に「同一要素×同一プロパティ系統」のペアがあると、チャンク順・詳細度の変化で勝敗が入れ替わる。長い祖先セレクタをモジュールルートに置換すると詳細度が下がり、共有CSSに負ける事故が典型(実測: カード幅-10px)
- 上書きペアは分割しない: 残置側がページ専用なら同居、共有なら移行側を差し戻す(ルート配下単位で。半々にしない)
- 検証は同一セッションの2ビルド比較(視覚回帰)で行う。撮影時期が違うとCMSコンテンツ差が混入する。委任先sandboxはlisten不可のため視覚回帰は司令塔の仕事

### 第2段階: 全ページModules化(class-map方式)

「凍結HTML・CMS本文とのクラス共有」と「境界ペア」は第1段階では移行不可の理由だが、次の機構で突破できる(実測: surface CSS 8ファイル・3バッチ計1,770ルールを完全移行し全ファイル0 byte化、視覚回帰実差ゼロ):

- **class-map(凍結HTML/CMS本文クラスの描画時書き換え)**: モジュール側がクラスの新しい正本になり、凍結HTML/CMS本文の class 属性を書き換えて追随させる(スタイル複製はしない=所有者は常にモジュール1つ)。書き換えはルートクラス相当だけで足りる(内部クラスは `:global()` でDOM上そのまま)。適用はサニタイズ後のclass属性変換として LegacyHtml と生HTML注入経路に一元化し、**ビルド時でなく描画時に変換**する(ドラフトプレビューのCSRでも同一コードが効く)。静的マップ(`src/lib/legacy-class-map.ts`)+単体テストで管理
- **勝者等価判定**: 境界ペア=即ブロッカーにせず、(specificity, 出現順)から移行前後のcascade勝者を静的計算し、**勝者が変わらないペアは移行可**とする。勝者が変わるものだけ (a)競合ルールのowner側への同居移行 (b)`:where()`によるspecificity保存変換 で解消(実測: 1バッチ最大1万ペア超を全て機械判定、解消不能ゼロ)
- **owner一元化**: 複数テンプレートが共有するルートクラスは最頻使用テンプレのModuleを単一ownerに決め、他コンポーネントは同じModuleをimport共有(ルールの複製禁止)。class-mapへの登録も1件だけ持つ
...
- **最終形: テーマディレクトリ完全撤去(実測で完遂可能)**: 発注があれば `wp-content/themes/<theme>/` を消滅させるところまで進められる。(1) グローバル基盤CSSは owner 確定クラスタだけModules抽出し、残置は**原文順序保存・出典コメント付きの `legacy-base.css`** としてチェックイン。vendor CSS(slick/lightbox等)は相対url()のフォント・画像ごと `src/styles/vendor/` へ同梱(css-loaderがハッシュ配信)。生成バンドルは「テーマ以外の前置(WPプラグイン等)」と「外部フォント」の**前後2分割**にし、layout の import 順(pre→base→vendor→fonts→補正CSS)で**元ソース順序を機械検証付きで保存**する。(2) テーマ画像は `public/assets/` へバイト同一移設し、media manifest の **local 値だけ**更新(キー=原URLは識別子として不変。HTML/RSC/faviconはmanifest解決で自動追随)。CSS url() は機械置換。(3) 固定refとAST照合する検証器には新旧パスの**単一正規化ヘルパー**を1箇所に実装して比較直前に適用(スクリプトごとの場当たり置換は禁止)。ゲートは「配信物(html/css/RSC)の旧テーマパス参照0」— JSチャンク内のmanifestキー・凍結原文データは許容と明記する(無条件grepはキー不変原則と両立しない)。撤去後に凍結HTML内の旧テーマCSSへの `<link>` がdead参照として残るのでsanitize側で除去する

### 成果物

- `docs/css-ledger.md`(所有権台帳)、`docs/css-batchN-*-audit.md`(境界監査・owner内訳の全列挙)、`docs/css-modules-mechanism.md`(class-map・勝者等価判定の仕様正本)
- 完了の定義: 第1段階のみなら「変更頻度の高い部分がModulesに移り、残したlegacyが台帳記録された互換層であること」。第2段階まで発注された場合は上記完了ライン(surface全量0 byte+guard撤去)
